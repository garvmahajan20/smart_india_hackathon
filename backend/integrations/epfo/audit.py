...
class EPFOAuditLogger:
    """
    Thread-safe deterministic audit logger with bounded memory buffer for EPFO calls.
    """
    def __init__(self, max_buffer_size: int = 1000):
        self.max_buffer_size = max_buffer_size
        self._events: List[EPFOAuditEvent] = []
        self._lock = Lock()
        self._seq = 0
    ...
    def log(
        ...
        with self._lock:
            self._seq += 1
            event_id = f"EPFO-AUDIT-{self._seq:06d}"
            safe_details = self._sanitize(details or {})

            event = EPFOAuditEvent(
                event_id=event_id,
        ...
        for k, v in data.items():
            ...
            if lower_k.endswith("_hash"):
                ...
                if isinstance(v, str) and v:
                    ...
                    scrubbed[k] = "[REDACTED]"
            elif any(ik in lower_k for ik in ("uan", "scno", "ppono", "identifier")) and isinstance(v, str):
                scrubbed[k] = mask_identifier(v)
            elif "dob" in lower_k and isinstance(v, str):
                scrubbed[k] = "**-**-****"
...
default_epfo_audit_logger = EPFOAuditLogger()

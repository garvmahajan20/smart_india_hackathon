...
const decisionMeta: Record<Decision, { label: string; description: string }> = {
  CONFIRM: {
    ...
    description: "Keep the escalation active as a substantiated finding.",
  },
  DISMISS: {
    label: "Dismiss finding",
    description: "Record that the officer does not consider the finding actionable.",
  },
  CLARIFICATION: {
    label: "Request clarification",
    description: "Send the bidder back for additional supporting evidence.",
  },
};
...
export const ReviewQueuePage: React.FC = () => {
  ...
  const recordDecision = () => {
    ...
    setDecisions((prev) => ({
      ...prev,
      [selectedReview.review_id]: selectedDecision,
    }));
    setSavedMessage(
  ...
  const columns = useMemo<ColumnDef<ReviewRow>[]>(
    () => [
      ...
      },
      {
        ...
        header: ({ column }) => (
          <button
            ...
            className="inline-flex items-center gap-1 text-[11px] font-semibold uppercase text-slate-700 hover:text-slate-900"
          >
            Review ID / Bid
        ...
        cell: ({ row }) => {
          ...
          return (
            <div>
              ...
              <span className="block font-mono text-[11px] text-blue-600">
                {row.original.bid_id}
              </span>
              {decision && (
                <span className="mt-1 inline-flex items-center gap-1 rounded bg-slate-100 px-1.5 py-0.5 text-[9px] font-bold uppercase text-slate-600">
      ...
      },
      {
        ...
        header: () => <span className="text-[11px] uppercase text-slate-600">Severity</span>,
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <p className="max-w-sm text-xs leading-snug text-slate-700">{row.original.reason}</p>
        ),
      },
      {
        ...
        cell: ({ row }) => (
          <div className="flex items-center gap-1 text-[11px] font-mono text-slate-500">
            ...
            <span className="max-w-[150px] truncate">{row.original.source_documents.join(", ")}</span>
          </div>
        ),
      },
      {
        ...
        cell: ({ row }) => (
          <div className="flex items-center justify-end gap-1.5">
            <button
              type="button"
              onClick={() => openAdjudication(row.original)}
              className="inline-flex items-center gap-1 rounded bg-slate-900 px-2.5 py-1 text-xs font-semibold text-white shadow-sm transition-colors hover:bg-slate-800"
            >
            ...
            <Link
              ...
              className="inline-flex items-center gap-1 rounded border border-slate-200 bg-white px-2.5 py-1 text-xs font-semibold text-slate-700 transition-colors hover:border-slate-300 hover:text-slate-950"
            >
              Investigate
      ...
      <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 bg-slate-50 p-3">
          ...
          {selectedCount > 0 && (
            <div className="flex items-center gap-2 rounded-lg border border-blue-200 bg-blue-50 px-3 py-1.5 text-xs text-blue-900">
              ...
              <span className="text-blue-400">Bulk actions intentionally require individual adjudication.</span>
            </div>
          )}
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[1050px] text-left text-xs">
            <thead className="border-b border-slate-200 bg-slate-100/80 text-slate-700">
